...
def Particles(name="Particles", rotation=None, translation=None, color=None):
    ...
    # particles.addObject('EulerImplicitSolver')
    self.addObject('CGLinearSolver', name='LinearSolver')
    self.addObject("MechanicalObject", name="MModel")
    self.addObject("RegularGridTopology",
                        nx=5, ny=40, nz=5,
                        xmin=-1.5, xmax=0,
                        ymin=-3, ymax=12,
                        zmin=-1.5, zmax=0,
                        drawEdges=1)
    self.addObject("UniformMass", name="M1", vertexMass=1)
    self.addObject("SpatialGridContainer", cellWidth=0.75)
    self.addObject("SPHFluidForceField", radius=0.745,
                        density=15, kernelType=1, viscosityType=2,
                        viscosity=10, pressure=1000, surfaceTension=-1000)
    self.addObject("PlaneForceField", name='p1', normal=[1, 0, 0], d=-4, showPlane=1)
    self.addObject("PlaneForceField", name='p2', normal=[-1, 0, 0], d=-4, showPlane=1)
    self.addObject("PlaneForceField", name='p3', normal=[0.5, 1, 0.1], d=-4, showPlane=1)
    self.addObject("PlaneForceField", name='p4', normal=[0, 0, 1], d=-4, showPlane=1)
    self.addObject("PlaneForceField", name='p5', normal=[0, 0, -1], d=-4, showPlane=1)
...
def createScene(rootNode):
    ...
    # confignode.addObject('OglSceneFrame', style="Arrows")
    ...
    # scene = Scene(rootNode, gravity=[0.0, -10.0, 0.0], dt=0.0001,
